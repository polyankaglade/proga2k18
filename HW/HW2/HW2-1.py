...
# инструмент: функция сохраняет полученные данные о репозиториях каждого юзера из списка в отдельный файл
# она используется вместо того, чтобы для каждого из подсчетов делать запрос на сервер
# это сокращает время выполнения программы
# можно было бы не использовать запись в файл, а записать все данные в одну переменную,
# но это вроде бы медленнее
def get_all_data(userlist, token):
...
# инструмент: функция берет данные юзера из полученного выше файла и записывает их в переменную
def read_data(user):
    filename = '%s.txt' % user
...
# Блок 1: технический
    ...
    token = 'fb87841f843337df0f5d242f2c4f9ffc48505bc3' # ВСТАВЬТЕ СВОЙ ТОКЕН СЮДА
...
# Блок 3: работа со всеми юзерами из списка
    get_all_data(userlist, token) # все данные об их репозиториях выкачиваются в файлы для упрощения работы с ними
    count_repos(userlist) # подсчет у кого из пользователей в списке больше всего репозиториев
    count_langs(userlist) # подсчет какой язык самый популярный среди пользователей списка
