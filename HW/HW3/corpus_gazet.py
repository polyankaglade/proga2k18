...
# не дает запустить рограмму при неверном пути к майстему
def check_mystem():
    if not os.path.exists(r'C:\mystem.exe'):
        print('MyStem лежит не в корневой папке.')
        print('Переместите его туда или измените путь к нему в коде программы' +
              '(функция mystem(), 137 строка) и перезапустите её.')
        sys.exit()
